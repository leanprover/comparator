...
def verifyMatch (challengeExport : String) (solutionExport : String) :
  ...
  let targets := theoremNames ++ (← getLegalAxioms)
...
def compareIt : M Unit := do
  ...
  let exportTargets := (← builtinTargets) ++ (← getTheoremNames) ++ (← getDefinitionNames)
    ++ (← getLegalAxioms) ++ (← primitiveTargets)
...
structure Config where
  ...
  /--
  Names of `def`s in the challenge whose body may be filled in by the solution.
  Optional; defaults to none. The solution must declare each listed name as a
  `def` with the same type, safety, and mutual-group list; only the body (and
  reducibility hints) may differ.
  -/
